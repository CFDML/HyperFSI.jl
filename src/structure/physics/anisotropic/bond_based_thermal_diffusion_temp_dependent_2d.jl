...
:Gc, :epsilon_c, :kcx, :kcy, :aph, :cv, :rft, :h, :hσ, :hϵ, :tem∞, :thick, :kp_Tx, :kp_Ty, :cv_T, :aph_T)
...
function BBTTxyPointParameters(mat::BBTTxyMaterial, p::Dict{Symbol,Any})
    ...
    if haskey(p, :thick) #2D 
        thick = float(p[:thick])
        kpx = (9*kcy -3*kcx)/ (π *  thick * δ^3) # microcndicitvity constant
        kpy = (12*kcx -12*kcy)/ (π *  thick * δ^3) # microcndicitvity constant
        kp = 6 * max(kcx, kcy) / (π *  thick * δ^3)
        bc = 9 * E / (π * thick * δ^3)      
    else #3D
...
function pflux_point!(storage::BBTTxyStorage, system::Peridynamics.BondSystem, 
    ...
    for bond_id in system.bond_ids[i]
        ...
        storage.pflux[1, i] += storage.bond_active[bond_id] *  (Δtem / L) * kp_tm * system.volume[j]
    end
    return nothing
end
...
function pflux_point!(storage::BBTTxyStorage, system::Peridynamics.BondSystem, 
    ...
    for bond_id in system.bond_ids[i]
        ...
        storage.pflux[1, i] += storage.bond_active[bond_id] * kp_tm * (Δtem / L) * system.volume[j]
    end
    return nothing
end
